...
export function buildWhatsAppUrl(phoneNumber: string): string {
  const phone = process.env.NEXT_PUBLIC_ADMIN_WHATSAPP ?? "573226370671";
  const message = encodeURIComponent(
    `Hola! Acabo de enviar el pago de mi pase diario. Mi numero de telefono registrado es: ${phoneNumber}`
  );
